...
export class CommentCache extends BaseCache {
  ...
  public async getCommentsNamesFromCache(postId: string): Promise<ICommentNameList[]> {
    try {
      ...
      const commentsCount: number = await this.client.LLEN(`comments:${postId}`); //give us the total number of items inside the specific comments lists with this postID
  ...
  public async getSingleCommentFromCache(postId: string, commentId: string): Promise<ICommentDocument[]> {
    try {
      ...
      const comments: string[] = await this.client.LRANGE(`comments:${postId}`, 0, -1);  //get all the comments
      const list: ICommentDocument[] = [];
      for (const item of comments) {
        list.push(Helpers.parseJson(item));
      }
      //find a particular comment matches the commentID
