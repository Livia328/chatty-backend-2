variable "aws_region" {
  ...
  type        = string
  default     = "us-west-1" # Your AWS region
}
...
variable "vpc_availability_zones" {
  ...
  type        = list(string)
  default     = ["us-west-1b", "us-west-1c"] # Two availability zones for your specific region
}
...
variable "bastion_host_cidr" {
  ...
  type        = string
  default     = "10.0.0.150/32" # Your <IP address>/32
}
...
variable "main_api_server_domain" {
  ...
  type        = string
  default     = "liviachenserver.online" # Your backend domain you created a route53 zone for
}

variable "dev_api_server_domain" {
  description = "Dev API Server Domain"
  type        = string
  default     = "api.dev.liviachenserver.online"
}
...
variable "ec2_iam_role_policy_name" {
  ...
  type        = string
  default     = "chatapp-server-ec2-policy" # Add a unique name
}

variable "ec2_instance_profile_name" {
  description = "EC2 Instance Profile Name"
  type        = string
  default     = "chatapp-server-ec2-role-profile" # Add a unique name
}
...
variable "code_deploy_role_name" {
  ...
  type        = string
  default     = "chatapp-server-code-deploy-role" # Add a unique name
}
