...
export class PostCache extends BaseCache {
  ...
  public async getPostsFromCache(key: string, start: number, end: number): Promise<IPostDocument[]> {
    try {
      ...
      const reply: string[] = await this.client.ZRANGE(key, start, end, { REV: true }); //返回cache中最新的
      const multi: ReturnType<typeof this.client.multi> = this.client.multi();
  ...
  public async deletePostFromCache(key: string, currentUserId: string): Promise<void> {
    try {
      ...
      const postCount: string[] = await this.client.HMGET(`users:${currentUserId}`, 'postsCount'); //get the post count
      const multi: ReturnType<typeof this.client.multi> = this.client.multi();
      multi.ZREM('post', `${key}`);
      multi.DEL(`posts:${key}`);
      multi.DEL(`comments:${key}`);
      multi.DEL(`reactions:${key}`);
      const count: number = parseInt(postCount[0], 10) - 1; //减少post的数量
      multi.HSET(`users:${currentUserId}`, ['postsCount', count]);
      await multi.exec();
    } catch (error) {
      log.error(error);
